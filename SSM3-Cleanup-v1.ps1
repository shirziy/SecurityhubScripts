<#
.SYNOPSIS
    Invokes various AWS Tools for PowerShell cmdlets to suppress SSM.3 Security Hub findings, as these are Low severity findings that are typcially the result of an anomaly with AWS Config/SSM - the findings often self-resolve and those that don't do not survive past a maintenance cycle, as the Instances are re-created during maintenance.
.DESCRIPTION
    This script is used to retrieve and suppress SSM.3 findings from Security Hub across all environments for the specified System.
.PARAMETER Org
    The system acronym
.PARAMETER EUAID
...
$NoteText="SSM.3 Findings are the result of an anomaly within SSM whereby a file is unreachable at the time that AWS Config attempts evaluate the rule.  These findings self resolve within a few days, and as they are a LOW severity are being suppressed as no action will be taken by the team."
$GID="aws-foundational-security-best-practices/v/1.0.0/SSM.3"
...
if($Auth.StatusCode -ne 200){
	...
	$PSProjects | %{
		...
		if($Count -eq 0){
			write-host "No SSM.3 Findings in $env.  Congratulations!"
		}Else{
			$message=$Count.toString()+" SSM.3 findings in "+$env
			write-host $message
			$Findings | %{
				$region=$_.Region
				$FID=$_.Id
