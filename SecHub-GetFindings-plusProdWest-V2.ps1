...
.NOTES
    ...
    Date:   June 22, 2022
	Version 2 Update 4/14/23 - Script includes us-west-2 for PROD environment to check for Security Hub findings in our PROD DR environment.  Added $PSCreds to parameters sent to arrayBuilder to facilitate retrieval of resource name tags.
	Version 7 Update 2/24/23 - added PROD us-west-2 as PROD DR environment for retrieval of findings
...
if($Auth.StatusCode -ne 200){
	...
	$PSProjects | %{
		...
		#for PROD, run an additional query to US-WEST-2 region for PROD DR
		if($env -eq "APS PROD"){
			$DRFindings = get-shubfinding -AccessKey $AccessKey -SecretKey $SecretKey -SessionToken $SessionToken -region US-WEST-2 -filter $Filter
			#If no findings exist, display a message on screen and continue to next environment
			if($DRFindings.count -eq 0){
				write-host "No Security Hub Findings in PROD WEST.  Congratulations!"
			}else{
				$message=($DRFindings.Count).toString()+" findings in PROD WEST."
				write-host $message
			#Call the ArrayBuilder to parse the findings into the iner array for the PROD WEST environment
			$InnerArray=ArrayBuilder -Findings $DRFindings -product SecHub -env "PROD WEST" -PSCreds $PSCreds
			#add PROD WEST findings processed to the external array
			$OuterArray+=$InnerArray
			}
		}
	}
